...
- Docker Compose for local development (PostgreSQL + FastAPI)
...
### MockBUParser
The `MockBUParser` implements the `BUParser` abstract interface. It generates deterministic vote data based on the MD5 hash of the file path. This allows the full system to operate before real PDF parsing is implemented. Replace only `MockBUParser` in Phase 05 — no other code changes required.
...
**Location**: `backend/app/parsers/base.py`
...
### MockBUParser
**Location**: `backend/app/parsers/mock_bu_parser.py`

Generates deterministic vote data using MD5 hash of the file path as a seed. Produces votes for 3 offices: Presidente, Governador, Senador. Replace this class in Phase 05 with a real PDF parser.

### BUService
**Location**: `backend/app/services/bu_service.py`
...
### API Endpoints
| Method | Path | Description | Frontend Service Method |
|---|---|---|---|
| GET | `/health` | Health check | — |
| GET | `/api/v1/boletins` | List all BUs | `getBoletins()` |
| POST | `/api/v1/upload` | Upload BU PDF | `uploadFile()` |
| GET | `/api/v1/dashboard/stats` | Dashboard statistics | `getDashboardStats()` |
| GET | `/api/v1/dashboard/coverage` | Section coverage | `getSectionCoverage()` |
| GET | `/api/v1/dashboard/timeline` | Upload timeline (last 24h) | `getUploadTimeline()` |
| GET | `/api/v1/results` | Aggregated vote results | `getAggregatedResults()` |
...
| s3_key | String(1024) | Nullable (Phase 06) |
...
2. Add business logic to `backend/app/services/bu_service.py` or create a new service.
...
5. Add Pydantic schemas in `backend/app/schemas/`.

### Replace MockBUParser with real PDF parser
1. Create `backend/app/parsers/real_bu_parser.py` implementing `BUParser`.
2. Override the `parse(file_path: str) -> ParsedBU` method.
3. In `backend/app/services/bu_service.py`, change the default parser:
   ```python
...
- **Timeline query uses SQLite-compatible ORM** — in production (PostgreSQL), consider using `date_trunc` for more accurate hourly grouping.
